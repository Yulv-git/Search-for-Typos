### 
 ...
 # @LastEditTime: 2022-05-02 23:36:55
...
do
    ...
    grep -r -i -w -n --color=auto "${item2}" ${target_dir} --exclude-dir=${exclude_dir}
...
do
    ...
    grep -r -n --color=auto "${item2}" ${target_dir} --exclude-dir=${exclude_dir}
...
do
    ...
    grep -r -i -n --color=auto "${item}" ${target_dir} --exclude-dir=${exclude_dir}
...
do
    ...
    egrep -r -n --color=auto "${item}" ${target_dir} --exclude-dir=${exclude_dir}
